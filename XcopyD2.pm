...
=head1 SYNOPSIS for XcopyD module
    ...
    $fx->fn_pat('(\.pl|\.txt)$');   # Lists extensions such as pl & txt that filter the files to be xcopied or moved
    ...
    # The 2 statements below statements apply only to xcopy
    ...
    # Execute xoopy as follows:
    $fx->xcopy;                    # or
    $fx->execute('xcopy');         # or
    $fx->xcp("from_dir", "to_dir", "file_name_pattern"); # the same with short name (not all parameters used)
...
How to use xcopy:
  ...
  Place this file/module (XcopyD.pm) is a folder named by its package name, VBmodules
  (You can rename this package in this file and place this file in a different folder of the same name as the package name)
  In the user perl script, place the following lines:
  use VBmodules::XcopyD;
...
1/10/2019: (vlb) - Merged 2 versions and tested xcopy.
 NEW: A summary of the xcopy results is the log file "xcopyDlog.txt" in the parent folder of the $from folder
 LIMITATIONS: File name with spaces in their path do not copy
 BUG: Some files mysteriously do not copy
